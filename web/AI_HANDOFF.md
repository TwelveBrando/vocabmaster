...
- Grammar tabs are freely selectable from the start. Each tab retains its current question, draft, selected/submitted answer and hint. All three types may be completed in any order; the final completed type can finish the quiz. Regenerating one type resets only that type. Browser smoke coverage includes tab switching and reverse-order completion.
