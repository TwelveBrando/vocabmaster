...
app.whenReady().then(async () => {
  ...
  const click = async label => {
    const found = await run(`(() => { const node = [...document.querySelectorAll('button')].find(b => b.textContent.trim() === ${JSON.stringify(label)}); if (!node) return false; node.click(); return true; })()`);
  ...
  try {
    ...
    console.log(JSON.stringify({ result: 'passed', checked: 'lecture entry, 28 answers, individual regeneration, 21-point persisted score, full regeneration, mobile width', score, errors }));
    app.exit(0);
  } catch (error) {
    console.error(error);
