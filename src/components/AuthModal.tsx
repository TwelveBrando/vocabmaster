...
export function AuthModal({ isOpen, onClose, onAuthenticated, theme: themeName }: { isOpen: boolean; onClose: () => void; onAuthenticated: (user: CloudUser) => Promise<void> | void; theme: UITheme }) {
  ...
  const submit = async (event: React.FormEvent) => { event.preventDefault(); setLoading(true); setError(''); try { const user = await cloudSyncService.signIn(email, password, register); void onAuthenticated(user); onClose(); } catch (e) { setError(e instanceof Error ? e.message : 'Ошибка авторизации.'); } finally { setLoading(false); } };
