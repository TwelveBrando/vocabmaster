...
export function AuthModal({ isOpen, onClose, onAuthenticated, theme: themeName }: { isOpen: boolean; onClose: () => void; onAuthenticated: (user: CloudUser) => void; theme: UITheme }) {
  ...
  const submit = async (event: React.FormEvent) => { event.preventDefault(); setLoading(true); setError(''); try { onAuthenticated(await cloudSyncService.signIn(email, password, register)); onClose(); } catch (e) { setError(e instanceof Error ? e.message : 'Ошибка авторизации.'); } finally { setLoading(false); } };
