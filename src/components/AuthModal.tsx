...
export function AuthModal({ isOpen, onClose, onAuthenticated, theme: themeName }: { isOpen: boolean; onClose: () => void; onAuthenticated: (user: CloudUser) => Promise<void> | void; theme: UITheme }) {
  ...
  return createPortal(<div className={`auth-layer theme-${themeName} fixed inset-0 z-[110] grid place-items-center p-4 sm:p-7`}>
    ...
    <section className="auth-panel relative grid w-full max-w-5xl overflow-hidden rounded-[2rem] border border-white/15 shadow-2xl md:grid-cols-[1.03fr_0.97fr]">
      <form autoComplete="on" onSubmit={submit} onKeyDownCapture={(event) => { if (event.key === 'Enter') event.stopPropagation(); }} className={`auth-form relative flex min-h-[570px] flex-col justify-center p-7 sm:p-12 ${theme.modalBg}`}>
        ...
        <div className="space-y-4"><label className={`block text-xs font-bold ${theme.textSecondary}`}>Email<input required name="email" type="email" autoComplete="email" value={email} onKeyDown={(event) => { if (event.key === 'Enter') { event.preventDefault(); event.currentTarget.form?.requestSubmit(); } }} onChange={(event) => setEmail(event.target.value)} placeholder="you@example.com" className={`mt-2 w-full rounded-2xl border p-4 text-sm outline-none transition ${theme.inputBg} ${theme.cardBorder} ${theme.inputText} ${theme.inputPlaceholder} ${theme.inputFocus}`} /></label><label className={`block text-xs font-bold ${theme.textSecondary}`}>Пароль<span className={`relative mt-2 flex rounded-2xl border ${theme.inputBg} ${theme.cardBorder} ${theme.inputFocus}`}><input required name="password" minLength={8} type={showPassword ? 'text' : 'password'} autoComplete={register ? 'new-password' : 'current-password'} value={password} onKeyDown={(event) => { if (event.key === 'Enter') { event.preventDefault(); event.currentTarget.form?.requestSubmit(); } }} onChange={(event) => setPassword(event.target.value)} placeholder="Минимум 8 символов" className={`w-full rounded-2xl bg-transparent p-4 pr-12 text-sm outline-none ${theme.inputText} ${theme.inputPlaceholder}`} /><button type="button" onClick={() => setShowPassword((value) => !value)} className={`absolute inset-y-0 right-3 grid w-9 place-items-center ${theme.textMuted}`} aria-label={showPassword ? 'Скрыть пароль' : 'Показать пароль'}>{showPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button></span></label></div>
